...
class AircraftMAMLSplit():
    def __init__(self, root, train=True, num_train_classes=70,
        ...
        if self._train:
            all_character_dirs = glob.glob(self.root + '/train/**') #return a list of path name that match "pathname"
...
class AircraftMetaDataset(object):
    ...
    def _get_aircraft_data_loader(self):
        ...
        batch_size = (self._num_classes_per_batch *   # N ways
                      self._total_samples_per_class * # k shorts
                      self._meta_batch_size)          # num of tasks in outer looop
